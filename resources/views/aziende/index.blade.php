...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center">
            <h1 class="h3 mb-0 text-gray-800 me-3">Aziende</h1>
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="viewSelector">
                    ...
                    <li><a class="dropdown-item" href="{{ route('companies.index') }}">Vista di Default</a></li>
                    @if($listViews->isNotEmpty())
                        @foreach($listViews as $view)
                        <li><a class="dropdown-item" href="{{ route('companies.index', ['view_id' => $view->id]) }}">{{ $view->name }}</a></li>
                        @endforeach
                    @endif
        ...
        <div class="btn-group" role="group">
            ...
            <a href="{{ route('companies.export') }}" class="btn btn-outline-secondary" data-bs-toggle="tooltip" title="Esporta CSV"><i class="bi bi-download"></i></a>
            <a href="{{ route('companies.create') }}" class="btn btn-success" data-bs-toggle="tooltip" title="Crea Nuova Azienda"><i class="bi bi-plus-lg"></i></a>
        </div>
    </div>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            @foreach($columns as $column)
                                <td>
                                    @if($column === 'assigned_to_id') {{ $company->assignedTo->name ?? 'N/D' }}
                                    @elseif($column === 'main_contact_id') {{ $company->mainContact->full_name ?? 'N/D' }}
                                    @else {{ $company->$column }} @endif
                                </td>
                            @endforeach
                            <td class="text-nowrap">
                                <form action="{{ route('companies.destroy', $company->id) }}" method="POST" onsubmit="return confirm('Sei sicuro?');" class="d-inline">
                                    <a href="{{ route('companies.show', $company->id) }}" class="btn btn-info btn-sm" data-bs-toggle="tooltip" title="Visualizza"><i class="bi bi-eye-fill"></i></a>
                                    <a href="{{ route('companies.edit', $company->id) }}" class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Modifica"><i class="bi bi-pencil-fill"></i></a>
                                    @csrf
                                    @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="{{ count($columns) + 1 }}" class="text-center">Nessuna azienda trovata.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
@include('partials.modal_customize_view', [
    'moduleClass' => 'App\\Models\\Company',
