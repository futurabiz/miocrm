...
<form action="{{ route('workflows.store') }}" method="POST" class="mt-3 card card-body">
    @csrf
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const savedData = {
        field: "{{ old('trigger_condition_field', '') }}",
        operator: "{{ old('trigger_condition_operator', '') }}",
        value: `{!! old('trigger_condition_value', '') !!}`,
        actionParams: @json(old('action_parameters', []))
    };
    ...
    function updateValueInput() {
        ...
        if (savedData.value) { const option = new Option(savedData.value, savedData.value, true, true); selectElement.append(option).trigger('change'); }
    }
    ...
    async function renderActionParams() {
        ...
        try {
            const url = `{{ route('workflows.getActionParameters') }}?action_type=${actionType}`;
