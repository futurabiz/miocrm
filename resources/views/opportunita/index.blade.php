...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center">
            <h1 class="h3 mb-0 text-gray-800 me-3">Opportunità</h1>
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="viewSelector">
                    ...
                    <li><a class="dropdown-item" href="{{ route('opportunities.index') }}">Vista di Default</a></li>
                    @if($listViews->isNotEmpty())
                        @foreach($listViews as $view)
                        <li><a class="dropdown-item" href="{{ route('opportunities.index', ['view_id' => $view->id]) }}">{{ $view->name }}</a></li>
                        @endforeach
                    @endif
        ...
        <div class="btn-group" role="group">
            ...
            <a href="{{ route('opportunities.export') }}" class="btn btn-outline-secondary" data-bs-toggle="tooltip" title="Esporta CSV"><i class="bi bi-download"></i></a>
            <a href="{{ route('opportunities.create') }}" class="btn btn-success" data-bs-toggle="tooltip" title="Crea Nuova Opportunità"><i class="bi bi-plus-lg"></i></a>
        </div>
    </div>
    ...
    <div class="card shadow mb-4">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            @foreach($columns as $column)
                                <td>
                                    @if($column === 'company_id') {{ $opportunity->company->name ?? 'N/D' }}
                                    @elseif($column === 'contact_id') {{ optional($opportunity->contact)->full_name ?? 'N/D' }}
                                    @elseif($column === 'assigned_to_id') {{ $opportunity->assignedTo->name ?? 'N/D' }}
                                    @elseif($column === 'amount') € {{ number_format($opportunity->amount ?? 0, 2, ',', '.') }}
                                    @elseif($column === 'closing_date' || $column === 'close_date') {{ optional($opportunity->close_date)->format('d/m/Y') }}
                                    @else {{ $opportunity->$column }} @endif
                                </td>
                            @endforeach
                            <td class="text-nowrap">
                                <form action="{{ route('opportunities.destroy', $opportunity->id) }}" method="POST" onsubmit="return confirm('Sei sicuro?');" class="d-inline">
                                    <a href="{{ route('opportunities.show', $opportunity->id) }}" class="btn btn-info btn-sm" data-bs-toggle="tooltip" title="Visualizza"><i class="bi bi-eye-fill"></i></a>
                                    <a href="{{ route('opportunities.edit', $opportunity->id) }}" class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Modifica"><i class="bi bi-pencil-fill"></i></a>
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr><td colspan="{{ count($columns) + 1 }}" class="text-center">Nessuna opportunità trovata.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="card-footer">{!! $opportunities->appends(request()->except('page'))->links() !!}</div>
        @endif
    </div>
</div>

{{-- Assicurati che il partial della modale sia la versione "pulita", solo HTML --}}
@include('partials.modal_customize_view', [
    'moduleClass' => 'App\\Models\\Opportunity',
